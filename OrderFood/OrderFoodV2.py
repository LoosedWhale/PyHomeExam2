...
class orderItem:
    def __init__(self, name, price, frame, quantity = 0):
        ...
        self.removeButton.configure(command=self.remove)

    def draw(self, frame):
        self.nameLabel = Label(frame, text=self.name, bg=bgc, fg=fgc)
        self.addButton = Button(frame, text="  +  ", bg=bgc, fg="Green", activeforeground=bgc, activebackground="Green")
        self.removeButton = Button(frame, text="   -   ", bg=bgc, fg="Red", activeforeground=bgc, activebackground="Red")
        ...
        self.LineLabel = Label(frame, text="___________", bg=bgc, fg=fgc)

        self.LineLabel.pack(side = TOP, fill=X)
        ...
        self.removeButton.pack(side = LEFT, fill=X)
        self.frame.pack()
...
def getTotal():
    ...
    formatTotalPrice = "Total: " + str(totalPrice) + " kr"  "\n"+  str(itemNames)
    ...
    if totalPrice == 0:
        ...
        OrderTextBox.configure(state='normal')
        OrderTextBox.delete(0.0, END)
        OrderTextBox.insert(0.0, formatTotalPrice)
        OrderTextBox.configure(state='disabled')
...
OrderButton = Button(frameFour, text="Order", command=getTotal, bg=bgc, fg="Gold", activeforeground=bgc, activebackground="Gold", font = (None, 10, "bold"))
OrderTextBox = Text(frameFour, height=26, width=20, bg=bgc, fg="Snow2") 
OrderTextBox.insert(END, "Total: ")
OrderTextBox.configure(state='disabled')

OrderButton.pack(side=BOTTOM, fill=X)
